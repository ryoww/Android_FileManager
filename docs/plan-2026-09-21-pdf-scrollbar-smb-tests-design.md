...
接続後の SMB 画面は「接続情報の行（`host / share`、"Connected"、編集・切断ボタン）」「ヘッダー」「ツールバー」の 3 段が一覧の上に積み重なり、画面上部の 2 割強を占めていた。接続情報の行が示す内容はヘッダーの副題に収まり、"Connected" は一覧が出ている時点で自明、編集・切断は毎回使う操作ではない。

- 接続情報の行（`SmbConnectedSummary`）を削除
- ヘッダー副題を `host / share › 現在のパス` の 1 行にし、長いパスは省略記号で切る
- 編集・切断はツールバー右端の「その他」メニュー（Edit connection / Disconnect）へ。`DropdownMenu` はボタンと同じ `Box` に入れないと Row の左端に出るので注意
...
### 追補: フォルダから戻ったとき一覧のスクロール位置を復元（同日）

一覧のスクロール状態を `currentPath` をキーにした `rememberSaveable` で作り直していたため、フォルダに入って「上へ」戻ると常に先頭に戻っていた。

フォルダのパスをキーにした `SaveableStateHolder.SaveableStateProvider` で `FileCollection` を包み、サブツリーの保存可能状態（`LazyGridState` / `LazyListState`）をフォルダごとに保持・復元する。新しいフォルダに入ったときはキーが初出なので先頭から、戻ったときは保存済みの位置になる。SMB 側は接続先（host / share）もキーに含め、別の共有に繋ぎ直したときに前の共有の位置を引き継がない。`AppNavHost` がルート画面の切り替えで使っているのと同じ仕組み。
